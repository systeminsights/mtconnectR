...
plot_twoway <- function(mtc_sim_mapped, mtc_device_sim, mtc_device, offset = 100,
  ...
  merged_mtc = merged_mtc %>% select(timestamp, mtc_map_string) %>% rename_("path_pos_x" = mtc_map_string)
  merged_sim = merged_sim %>% select(timestamp, sim_map_string) %>% rename_("x_pos" = sim_map_string)
